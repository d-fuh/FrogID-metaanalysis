---
title: "z5341711_BEES2041_Practical_Report_2_(Final Revised)"
output:
  html_document:
    df_print: paged
ID: z5341711
...
(1) Several footnotes and references are indexed under the #References and #Footnotes section at the end of the notebook, the footnotes are mostly technical elucidation on terms and statements. 

(2) [REF] refers to two citations: the papers by Callaghan et al. (2020) and Cogger (2018) from which the information were used in this report. [FOT] refers to a footnote. 

(3) Tables and figures used in this report are indexed in the #Tables & Figures section as well as in the navigation panel in the R notebook. 

(4) I recommend knitting the notebook to html format for optimal reading experience. 

(5) **Please note that the questions in this report are in different orders from the pdf instruction provided on Moodle.** Specifically, the last question on the pdf was put to the front (Question 1) here.

(6) There was confusion on the term *"number of species"* and I have revised the entire analyses of Q2 & 3 very shortly (a few hours) before the deadline after I came across the forum posts on Moodle. If anyone is interested in my rather detailed analyses on the "number of samples" recorded by frogID in lieu of Questions 2 & 3, they are welcome to read my previously submitted report. Hereafter the variable "number of species" refers to the **species richness** data exclusively.


# Introduction 

## (drawn from BEES2041 Moodle; for reader's reference only)
...
## a)	Notes to introduce the question and detail the hypotheses being tested (1-2 paragraphs) [10 marks]
...
(2) How does the number of species (=species richness) recorded by FrogID vary among ecoregions?^[FOT 1] 

Note that the number of species is a measure of species richness in a grid cell recorded by both datasets.

(3) Is the number of species recorded by FrogID affected by the remoteness of the sampling area?^[FOT 2]

We will address these questions in the order of (1)->(2)->(3) in the following analyses. 

The respective *null hypotheses* being tested in this report are: (1) H0: Sample mean of species richness from FrogID equals to that in the previous data sets derived from the work of Dr. Hal Cogger (“Cogger” hereafter). (2) H0: The means of number of species recorded by FrogID does not vary among different ecoregions. (3) H0: The number of species recorded by FrogID has no statistically significant relationship with the remoteness of the sampling area.

With the respective *alternative hypotheses*: (1) Sample mean of species richness from FrogID differs from Cogger data. (Notice that this is a two-tailed hypothesis.) (2) In at least two ecoregions from the study, the means of number of species recorded by FrogID vary. (3) The number of species recorded by FrogID has a statistically significant linear relationship with the remoteness of the sampling area under log transformation of both variables.

Interestingly --- In Question 1, we investigate whether the FrogID data accurately predict Cogger's by testing whether the two sample means differ significantly, and that if a significant difference is detected we say that FrogID failed to predict Cogger data accurately. This means that the traditional "null hypothesis" holds true (i.e. nothing happens, the two samples come from the same population) when FrogID makes an accurate prediction (i.e. in a sense, "something happens"). I am keeping the hypotheses as-is here considering that there may be confusion if I define the null hypothesis with respect to the research question (Q1) instead of the t-test, as is done traditionally. This notion is used throughout the rest of this report.
...
```{r, warning=FALSE}
library(tidyverse) #For various functions required hereafter
...
### Step 1. Download the required data files: cogger_richness_per_grid.csv, frogid_richness_per_grid.csv, into the same folder as this R notebook.

### Step 2. Read in the files in R:
```{r}
...
```
where rpg stands for richness per grid.


### Step 3. Data reorganisation for Question 1, Part D & F
...
(Q: Is there a faster way of achieving the exact data arrangement in R?)


## d) Notes and code to create a graph that can visualise how the data addresses the question [25 marks]

For each of the three questions presented in this report, we will generate a corresponding graph most suitable to represent the data to address the question at hand.

### Question 1: See Fig. 1, 2, 3a, 3b
...
The questions asks whether FrogID species richness accurately "predict" the well-documented species richness data by Dr. Cogger. In this report, to measure the accuracy of prediction, we examine how closely the two sample means in FrogID and Cogger species richness match statistically.

Since species richness is a single continuous variable, and we're trying to measure to what level do FrogID species richness predict Cogger, the idea of creating a scatter plot using species richness in both datasets and measuring its R-square is tempting. However, since the number of observations for species richness differs in FrogID and Cogger (N=2746 for Cogger and 577 for FrogID), we cannot find a number n, to generate a scatter plot with n points representing species richness in the two data sets.

Therefore, we will use more feasible and visually expressive approaches to show the relationship of FrogID species richness and Cogger: In the following, we generate a series of box plot, *overlapping density plot*, and two bar graphs to compare which presentation is the most suitable for examining how well the predictor (species richness in FrogID) matches the response (species richness in Cogger), and which graphs and the fittest to address our question at hand, given the criterion of accurate prediction.
...
From the graph, we see that the density of species richness in FrogID does resemble the Cogger data. Notice the two modes are distinct with little overlaps. **We suspect the true means in two groups are statistically different.** Based on our measure of accuracy of prediction (by matching sample means), this provides modest evidence against FrogID species richness accurately predicting Cogger species richness. However, further analyses are required.
...
Figure_3a <- ggplot(SR.barplot, aes(x = Data_type, y = Mean)) + 
  ...
  geom_errorbar( aes(x=Data_type, ymin=Mean-Se, ymax=Mean+Se), width=0.3, colour="orange", alpha=0.9, size=1.3) +
  ggtitle("Fig. 3a  Bar plot for species richness in FrogID vs. Cogger, \n with standard errors")
...
Figure_3b <- ggplot(SR.barplot, aes(x = Data_type, y = Mean)) + 
  ...
  geom_errorbar( aes(x=Data_type, ymin=Mean-ic, ymax=Mean+ic), width=0.3, colour="orange", alpha=0.9, size=1.3) +
  ggtitle("Fig. 3b  Bar plot for species richness in FrogID vs. Cogger, \n with 95% confidence intervals")

Figure_3b
```

We see that the means differ quite significantly in FrogID and Cogger. We see that there is visably larger variance in FrogID from both figures, which might be due to its relatively smaller sample size. **We suspect FrogID does not accurately predict Cogger species richness according to our criterion, given the overall large sample sizes in both datasets** (N=2746 for Cogger, N=577 for FrogID). We will verify this conjecture in Part F, Question 1.




### Question 2: See (in particular) Fig. 5

> "How does the number of species recorded by FrogID vary among ecoregions?"

There are two important things to consider first: 

(1) There is one problematic datum with undefined ecoregion at row 172; 
...
These issues can be detected by creating a bar graph counting the number of data in each ecoregion in FrogID:

#### Fig. 4  Distribution of Ecoregions in FrogID
```{r}
# First, we see the names of ecoregions are too long that affected the graph presentation. We fix this by:
frogid.rpg <- mutate(frogid.rpg, Ecoregionp = str_wrap(frogid.rpg$Ecoregion, width = 10)) 

    ## This generates a new column storing "organised" names of ecoregions, named "Ecoregionp" (p for plots)
...
frogid.rpg %>%
  ...
  mutate(myaxis = paste0(Ecoregionp, "\n (n=", num, ")")) %>%
                                                                                          
# Generate a bar graph which counts the number of data in each ecoregion in FrogID
ggplot(aes(x = myaxis)) +
    geom_bar() + 
    #scale_fill_continuous(name = "Sample size (n)", low = "darkolivegreen1", high = "darkolivegreen") +
    theme_bw() + 
    labs(x="Ecoregions") -> Figure_4

#+ ggtitle("Fig. 4  Distribution of Ecoregions in FrogID") 

Figure_4
```

Note that there are exactly 7 non-trivial types of ecoregions in FrogID. Notice also there is evident under-representation in groups Montane Grasslands & Shrublands and Tropical & Subtropical Moist Broadleaf Forests.

To address the issue with undefined ecoregion, we remove the problematic row 172 by:
```{r}
frogid.rpg <- slice(frogid.rpg, -172) #Removes the row with N/A ecoregion
```

We expect the removal of this data point to have little effect on the overall data behaviour for the following analyses, as it contains relatively small samples. We can now address Question 2:

> "How does the number of species (=species richness) recorded by FrogID vary among ecoregions?"

...by visualising the spread of species richness across different ecoregions using box plot:
...
# Generate a box plot representing the spread of number of species (species richness) in each ecoregion in FrogID 
  ggplot(aes(x=reorder(Ecoregionp, num), y=Species_richness_frogid, fill=num)) + 
                          ## Reordering ecoregions by increasing sample size
  geom_boxplot() + 
...
#+ggtitle("Fig. 5  Box plot of the number of species across various ecoregions in FrogID") 
...
(1) The number of species distribution seems right-skewed in most ecoregions, except Tropical & Subtropical Moist Broadleaf Forests (near-normal looking) and Montane Grasslands & Shrublands --- those with relatively smaller sample sizes. This is seen from the asymmetry in the boxes with respect to the median line.

(2) There might not be homoskedasticity across ecoregions for species richness.

(3) We see varying species richness across ecoregions in FrogID. With highest median observed in T&SMBF and lowest in D&XS --- note that this does not consider the individual sample sizes in each group.
...
frogid.rpg %>%
  left_join(sample_size) %>%
  mutate(myaxis = paste0("n=", num)) %>%  
                      ## Adding sample size of each ecoregion to the x lab

  # Continue with the main plot:
  ggplot(aes(x=myaxis, y=Species_richness_frogid, fill=Ecoregionp)) +
    ...
    scale_fill_discrete(name = "Eco-region") +
    labs(y= "Species Richness", x = "Eco-region") -> Figure_6
...
```{r}
library(plotly)
violin_plot <- ggplotly(Figure_6)
htmlwidgets::saveWidget(violin_plot, file="violin_plot.html")
```
...
However, I don't think if this plot is particularly more illustrative than the box plot (Fig. 5).

In conclusion, **we suspect that the means of number of species indeed vary across ecoregions** from the Figures 5 & 6, with the greatest species richness observed in Tropical & Subtropical Moist Broadleaf Forests, and the lowest richness seen in Deserts & Xeric Shrublands.




### Question 3: See Fig. 7

> "Is the number of species recorded by FrogID (Y) affected by the remoteness (X) of the sampling area?"

We first visualise the relationship between the number of species (=species richness) and remoteness in FrogID using a scatter plot: 
(Note: Both continuous variables)
```{r, warning=FALSE}
#Scatter plot: Number of species vs Remoteness in FrogID
...
Due to the large scale of the remoteness variable, it is difficult to see if there is a direct linear relationship between the number of species and remoteness in FrogID. We need to transform the data.

We suspect a log-transformation is most suitable in this scenario. We can first inspect the frequency histogram of both variables and that of log-scaled variables to observe the change of their location and scale under the transformation:
...
## One would suspect the sqrt transform is more suitable as the number of species is a "counted" data
```
...
(Q: In practice, should one log-transform both variables, or transform only remoteness? Or transform remoteness with log and number of species with sqrt?)



#### Fig. 7  Scatter plot: Number of species against remoteness in FrogID, log-scaled for both variables
...
We first notice there is certain "stripe-ness" in the number of species versus remoteness. This is largely due to the fact that the number of species was a counted data (i.e. the values recorded were strictly positive integers). This explains the discreteness seen on the Y-axis.
...
## e) Notes to describe the analytical methods (1-2 paragraphs) [15 marks]


The first question, *“Does the data on species richness of frogs in a grid cell obtained from FrogID accurately predict the known species richness from previous expert data sets by Dr. Cogger?”*, was analysed using an **independent samples t-test**, in which we test for whether the difference between the sample mean of species richness from FrogID and that of the Cogger data is significantly non-zero. We assume that a significant difference means the prediction made by FrogID species richness to Cogger data was inaccurate.
...
Note: The "tables" are in an informal sense and are simply summaries of the tests compiled by R. These can be easily called by the codes included in Part F. The tables and their locations are indexed in the #Tables & Figures section at the end of the notebook as well as in the navigation panel in R notebook.
...
#### Recaptulating the question:

> Does the data on *species richness* of frogs in a grid cell ("SR") obtained from **FrogID** accurately predict the known species richness from previous expert data sets by Dr. **Cogger**?
...
#### Restating the alternative hypothesis:

> HA: Sample mean of species richness from FrogID differs from the previous datasets, i.e. Cogger data.

Notice this is a **two-tailed hypothesis**.

#### Conducting the test:

##### Table 1.  Independent t-test of species richness from FrogID vs. Cogger, with Welch approximation
```{r}
t.test(Species_richness ~ Data_from, data = SR.type.both, alternative = "two.sided", var.equal = F) -> Table_1
Table_1
```
The interpretation of the results is in Part G, Question 1.
...
#### Conducting an a priori test in order to check for assumptions:
...
The normality assumption is mildly violated as we see the residuals are not normally distributed --- it is slightly skewed to the right.
...
##### Re-verification of test assumptions under sqrt transformation:
```{r}
#ANOVA: logNS over Ecoregion
...
#### Conducting the formal test: Using the log-transformed variable, logNS
...
#### A priori linear model for assumption testing:
```{r}
...
##Note that logNS was previously assigned to frogid.rpg in Part F, Q2
```
...
#### Summary of the linear model: Number of species ~ Remoteness in FrogID
```{r}
summary(frogid.lm)
```
...
#### Summary of the linear model: log(Number of species) ~ log(Remoteness) in FrogID
...
#### Fitted equation from lm: log(Number of species) ~ log(Remoteness) in FrogID
...
## g) Notes to interpret the results (1-2 paragraphs) [15 marks]

### The first question

> “Does the data on species richness of frogs in a grid cell obtained from FrogID accurately predict the known species richness from previous expert data sets by Dr. Cogger (2018)?”

It should be first noted that we arbitrarily assumed normality of the species richness data in both data sets (FrogID & Cogger) despite significant results of departure from normality were obtained from various tests. Welch approximation was used for the independent t-test due to suspected heteroskedasticity of the species richness data. The p-value for the independent t-test was near zero (P < 2.2e-16; Table 1), and therefore the alternative hypothesis was accepted: We conclude that there is strong evidence showing that *almost surely* the true difference for the sample means of species richness obtained from FrogID and that from the Cogger data is non-zero. 
    
Based on our criterion for accuracy of prediction, the test result provides strong evidence for FrogID species richness not  predicting Cogger species richness.  This finding is supported by the overlapping density plots (Fig. 2) and bar plots of the species richness data (Fig. 3a & 3b).  This however is **inconsistent** with the finding by Callaghan et al. where it was concluded that “there was still strong evidence of FrogID species richness predicting Cogger species richness”.  [REF 1]
    
In the paper, Callaghan et al. employed a different approach to assess the accuracy of prediction by FrogID.  They had  conducted additional meta-analyses on the relative difference between FrogID and Cogger datasets whereas here, only the first-level difference was analysed using Student's t-test. In comparison, there are several caveats in the t-test approach used in this report for evaluating the accuracy of prediction by FrogID, mainly surrounding how exactly should we quantify the "accuracy” of prediction by FrogID to the Cogger species richness. We propose that the accuracy of prediction needs be more rigorously defined for this question in order to build a more powerful model for its assessment.
...
**Table 1.** Independent t-test of species richness from FrogID vs. Cogger, with Welch approximation (F-1)  
...
4. Let NS denote the number of species in FrogID and R the remoteness. The linear model tells us that 
